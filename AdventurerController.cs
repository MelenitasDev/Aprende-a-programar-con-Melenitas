...
namespace MelenitasDev
{
	public class AdventurerController : MonoBehaviour
	{
		...
		void Update ()
		{
			...
			// Detectamos si se presiona la tecla espacio...
			if (Input.GetKeyDown(KeyCode.Space))
			{
				...
				// Si no, comprobamos si el doble salto no ha sido usado...
				else if (!doubleJumpUsed)
				{
					// Ejecutamos el doble salto.
					DoubleJump();
				}
			}
			...
			// Si el jugador está quieto en el eje X, sin saltar y tocando el suelo...
			if (xInput == 0 && currentState != AdventurerState.Jumping && IsGrounded())
			{
				// Cambiamos el estado a "Idle".
				ChangeState(AdventurerState.Idle);
			}
			// Si está moviéndose en el eje X, tocando el suelo y sin saltar...
			else if (Mathf.Abs(xInput) > 0 && IsGrounded() && currentState != AdventurerState.Jumping)
			{
				// Cambiamos el estado a "Corriendo".
				ChangeState(AdventurerState.Running);
			}
			// Y si no está tocando el suelo y su velocidad en Y es negativa...
		...
		private void DoubleJump ()
		{
			...
			// Cambiamos el estado a "Saltando Doble".
			ChangeState(AdventurerState.DoubleJumping);
			
			// Marcamos que el doble salto ha sido usado para no poder usarlo infinitamente.
			doubleJumpUsed = true;
		}
		...
		private void ChangeState (AdventurerState newState)
		{
			...
			switch (newState)
			{
				...
				case AdventurerState.Jumping:
					...
					break;
				case AdventurerState.DoubleJumping:
